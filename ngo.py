    ...
    if selected_meals:
        ...
        if save_btn:
            if not raw_donor_name or not donor_phone or not meal_occasion:
                st.error(
                    "❌ કૃપા કરીને દાતાશ્રીનું નામ, મોબાઈલ નંબર અને જમણવારનું નિમિત્ત અવશ્ય ભરો."
                )
            else:
                ...
                booking_dict = insert_data
                booking_dict["id"] = last_id
        ...
        if submit_d:
            if not raw_d_name or d_amount <= 0:
                ...
                data = {
                    ...
                    "amount": d_amount,
                    "payment_mode": d_mode,
                    "utr_number": d_utr if d_utr else "N/A"
                }
        ...
        with st.form("letter_form", clear_on_submit=True):
            ...
            if submit_letter:
                if not outward_no or not recipient or not subject or not body_text:
                    ...
                    last_letter_id = res.data[0]['id']
                    
                    st.success("🎉 પત્ર સફળતાપૂર્વક સેવ થઈ ગયો છે!")
                    letter_dict = data
                    letter_dict['id'] = last_letter_id
                    render_html_letter(letter_dict)

    with tab_L2:
        st.write("##### 📂 સેવ થયેલા તમામ પત્રોનું ઇન્ડેક્સ (Index)")
        res_l = supabase.table("letters").select("id, outward_no, ref_no, letter_date, recipient, subject").order("id", desc=True).execute()
        all_letters = res_l.data
        if all_letters:
            letter_options = {
                f"જાવક નં: {l['outward_no']} | તારીખ: {fmt_date(l['letter_date'])} | વિષય: {l['subject']}": l['id']
            ...
            if sel_l_id:
                res_let = supabase.table("letters").select("*").eq("id", sel_l_id).execute()
                if res_let.data:
                    l_dict = res_let.data[0]
                    ...
                    if st.session_state["editing_letter_id"] == sel_l_id:
                        ...
                        with st.form(f"edit_letter_form_{sel_l_id}"):
                            e_outward = st.text_input("જાવક નંબર (Outward No.)", value=l_dict["outward_no"])
                            e_ref = st.text_input("સંદર્ભ નંબર (Ref No.)", value=l_dict.get("ref_no", ""))
                            try:
                                parsed_dt = datetime.strptime(l_dict["letter_date"], "%Y-%m-%d").date()
                            except:
                                parsed_dt = date.today()
                            e_date = st.date_input("પત્રની તારીખ", value=parsed_dt)
                            e_recipient = st.text_area("પ્રતિ (Recipient Address)", value=l_dict["recipient"])
                            ...
                            if update_btn:
                                u_data = {
                                    "outward_no": e_outward.upper(),
                                    "ref_no": e_ref.upper(),
                                    "letter_date": str(e_date),
                                    "recipient": e_recipient,
                                    "subject": e_subject,
                                    "body_text": e_body
                                }
                                supabase.table("letters").update(u_data).eq("id", sel_l_id).execute()
                                st.session_state["editing_letter_id"] = None
...
def render_dashboard_module():
    if not is_admin:
        ...
        c_jmn = sum(item.get("amount", 0) for item in supabase.table("bookings").select("amount").execute().data)
        c_don = sum(item.get("amount", 0) for item in supabase.table("donations").select("amount").execute().data)
        tot_inc = c_jmn + c_don
        tot_exp = sum(item.get("amount", 0) for item in supabase.table("expenses").select("amount").execute().data)
        ...
        # --- 1. Upcoming 5 Days Meal Summary ---
        ...
        for ud in upcoming_dates:
            res_ud = supabase.table("bookings").select("id, donor_name, phone, service_for_name, meal_types, meal_prep_type, occasion, amount, payment_status").eq("booking_date", ud).execute()
            ud_bookings = res_ud.data
            
            st.write(f"**📅 {fmt_date(ud)}** ({'આજ' if ud == str(today_date) else 'આગામી દિવસ'})")

            if ud_bookings:
                for b in ud_bookings:
                    b_id, b_donor, b_phone, b_service, b_meal, b_prep, b_occ, b_amt, b_pstat = (
                        b["id"], b["donor_name"], b["phone"], b["service_for_name"], b["meal_types"], b["meal_prep_type"], b["occasion"], b["amount"], b["payment_status"]
                    )
                    ...
                    with col_w_btn:
                        ...
                        if not clean_p.startswith("91"): clean_p = "91" + clean_p
                        w_text = (
                            f"*NARMADESHWAR VIKLANG VIKAAS MANAV SEVA TRUST*%0A"
                            f"* જમણવાર બુકીંગનો કનફર્મેશન મેસેજ*%0A%0A"
                            f"નમસ્તે,%0A"
        ...
        with col_f1:
            filter_from_date = st.date_input("શરૂઆતની તારીખ (From Date)", value=date.today(), key="f_date")
        with col_f2:
            filter_to_date = st.date_input("સમાપ્તિની તારીખ (To Date)", value=date.today() + timedelta(days=7), key="t_date")

        if filter_from_date <= filter_to_date:
            res_range = supabase.table("bookings").select("id, donor_name, phone, service_for_name, booking_date, meal_types, meal_prep_type, occasion, amount").gte("booking_date", str(filter_from_date)).lte("booking_date", str(filter_to_date)).order("booking_date", desc=False).execute()
            range_bookings = res_range.data
            
            st.info(f"📌 {fmt_date(filter_from_date)} થી {fmt_date(filter_to_date)} દરમિયાન કુલ {len(range_bookings)} જમણવાર નોંધાયેલ છે.")
            if range_bookings:
                for b in range_bookings:
                    b_id, b_donor, b_phone, b_service, b_date, b_meal, b_prep, b_occ, b_amt = (
                        b["id"], b["donor_name"], b["phone"], b["service_for_name"], b["booking_date"], b["meal_types"], b["meal_prep_type"], b["occasion"], b["amount"]
                    )
                    col_info, col_w_btn = st.columns([4, 1])
                    with col_info:
                        st.markdown(
        ...
        # --- 3. Old Receipt / Donation Printing & Payment Update ---
        st.write("### 📄 જૂની પાવતી / રસીદ પ્રિન્ટ કરો & પેમેન્ટ અપડેટ (Mark as Paid)")
        rec_tab1, rec_tab2 = st.tabs(["🍲 જમણવાર રસીદો (Bookings)", "🎁 દાન રસીદો (Donations)"])

        with rec_tab1:
            res_all_b = supabase.table("bookings").select("id, donor_name, booking_date, payment_status").order("id", desc=True).execute()
            all_b = res_all_b.data
            if all_b:
                b_options = {
                    f"રસીદ નં #{b['id']} - {b['donor_name']} (તારીખ: {fmt_date(b['booking_date'])}) [Pay: {b['payment_status']}]": b['id']
                    for b in all_b
                }
                ...
                if sel_b_id:
                    res_rec = supabase.table("bookings").select("*").eq("id", sel_b_id).execute()
                    if res_rec.data:
                        receipt_dict = res_rec.data[0]
                        if "No" in str(receipt_dict["payment_status"]):
                            st.warning("⚠️ આ બુકિંગનું પેમેન્ટ હજુ બાકી (Pending) છે.")
                            if st.button("💰 પેમેન્ટ ચૂકતે કરો (Mark as Paid)", key=f"mark_paid_{sel_b_id}"):
                                supabase.table("bookings").update({"payment_status": "Yes (આવી ગયેલ છે)"}).eq("id", sel_b_id).execute()
        ...
        with rec_tab2:
            res_all_d = supabase.table("donations").select("id, donor_name, amount, created_at").order("id", desc=True).execute()
            all_d = res_all_d.data
            if all_d:
                d_options = {
                    f"દાન નં #{d['id']} - {d['donor_name']} (₹{d['amount']})": d['id'] for d in all_d
                }
                ...
                if sel_d_id:
                    res_d = supabase.table("donations").select("*").eq("id", sel_d_id).execute()
                    if res_d.data:
                        d_rec = res_d.data[0]
                        st.success(f"🎁 દાતા: {d_rec['donor_name']} | પ્રકાર: {d_rec['donation_type']} | રકમ: ₹{d_rec['amount']:,.2f} | મોડ: {d_rec['payment_mode']}")
            else:
        ...
        res_full = supabase.table("bookings").select("*").order("id", desc=True).execute()
        df_b = pd.DataFrame(res_full.data)
        
        if not df_b.empty:
            df_b = df_b[["id", "donor_name", "service_for_name", "occasion", "phone", "booking_date", "meal_types", "meal_prep_type", "amount", "payment_status", "payment_type"]]
            df_b.rename(columns={
                "id": "ID", "donor_name": "દાતાશ્રી", "service_for_name": "જેમના નામે સેવા", 
                "occasion": "નિમિત્ત", "phone": "મોબાઈલ", "booking_date": "તારીખ", 
                "meal_types": "જમણવાર", "meal_prep_type": "પ્રકાર", "amount": "રકમ (₹)", 
                "payment_status": "Pay Status", "payment_type": "Pay Type"
            }, inplace=True)
        ...
        with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
            if not df_b.empty: df_b.to_excel(writer, sheet_name="Jamanvar", index=False)
            
            df_donations = pd.DataFrame(supabase.table("donations").select("*").execute().data)
            if not df_donations.empty: df_donations.to_excel(writer, sheet_name="Donations", index=False)
            
            df_expenses = pd.DataFrame(supabase.table("expenses").select("*").execute().data)
            if not df_expenses.empty: df_expenses.to_excel(writer, sheet_name="Expenses", index=False)
            
            df_inventory = pd.DataFrame(supabase.table("inventory").select("*").execute().data)
            if not df_inventory.empty: df_inventory.to_excel(writer, sheet_name="Inventory", index=False)
            
            df_letters = pd.DataFrame(supabase.table("letters").select("*").execute().data)
            if not df_letters.empty: df_letters.to_excel(writer, sheet_name="Letters", index=False)
